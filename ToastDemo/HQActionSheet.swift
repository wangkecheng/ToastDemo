...
class HQActionSheet:UIView{
    ...
    var otherActionItemAlignment:HQOtherActionItemAlignment?  = .HQOtherActionItemAlignmentCenter//Default is SROtherActionItemAlignmentCenter when no images. SROtherActionItemAlignmentLeft when there are images.
...
//      sheet.cover?.backgroundColor = UIColor.red
//      sheet.cover?.alpha = 1.0
        sheet.alpha = 1.0
        sheet.actionSheet?.frame =  CGRect.init(x: 0, y: sheet.frame.height, width: sheet.frame.width, height: sheet.offsetY!)
    ...
    func setupCover() {
        ...
        cover.backgroundColor?.withAlphaComponent(0.33)
        ...
        if (self.otherTitles?.count ?? 0) > 0{
            for i in 0..<(self.otherTitles?.count ?? 0)  {
                ...
                if (self.otherImages?.count ?? 0) > 0{
                    ...
                    let title:UILabel  = UILabel.init(frame: CGRect.init(x: icon.frame.maxX, y: icon.frame.maxY, width: maxTitleSize.width, height: kActionItemHeight))
    ...
    func showWithSuperVC(VC:AnyObject) {
        ...
        UIView.animate(withDuration: 0.5, delay: 0.0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.7, options: UIViewAnimationOptions.curveEaseInOut, animations: { [unowned self] in
            ...
            self.actionSheet!.transform =   CGAffineTransform.init(translationX: 0, y: self.actionSheetHeight!)
        }, completion: nil)
    }
    @objc func dismiss(){
